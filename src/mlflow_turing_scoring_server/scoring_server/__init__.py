...
def init(model: PyFuncModel):
    ...
    @app.route("/turing", methods=["GET"])
    def turing():  # pylint: disable=unused-variable
        status = 200
        return flask.Response(response="{ turintech: test}", status=status, mimetype="application/json")
